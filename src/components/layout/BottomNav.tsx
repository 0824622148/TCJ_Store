...
const navItems = [
  ...
  { href: "/", icon: Home, label: "TCJ", isCenter: true },
          ...
          if (item.isCenter) {
            return (
              <Link key={item.href} href="/home" className="relative -mt-6">
                <motion.div
                  whileTap={{ scale: 0.9 }}
                  className="w-14 h-14 rounded-full bg-forest-gradient flex items-center justify-center shadow-[0_8px_24px_rgba(22,13,46,0.8)] border border-sage/30 p-0.5"
                >
                  <Image src="/images/tcj-logo.png" alt="The Cloud Junction" width={48} height={48} className="object-contain rounded-full" />
                </motion.div>
              </Link>
            );
          }
